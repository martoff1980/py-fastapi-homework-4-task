...
async def test_registration(e2e_client, reset_db_once_for_e2e, settings, seed_user_groups, e2e_db_session):
    ...
    assert validate_url(activation_url, simple_host=True), f"The URL '{activation_url}' is not valid!"
...
async def test_account_activation(e2e_client, settings, e2e_db_session):
    ...
    assert validate_url(login_url, simple_host=True), f"The URL '{login_url}' is not valid!"
...
async def test_request_password_reset(e2e_client, e2e_db_session, settings):
    ...
    assert validate_url(reset_link, simple_host=True), f"The URL '{reset_link}' is not valid!"
